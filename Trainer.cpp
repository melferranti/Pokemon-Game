...
void Trainer :: StartBattling(unsigned int num_battles)  {
    if (current_gym == NULL || GetDistanceBetween(this -> location, current_gym -> GetLocation()) != 0)   {
...
void Trainer :: StartRecoveringHealth(unsigned int num_potions)  {   
    if (current_center == NULL || GetDistanceBetween(this -> location, current_center -> GetLocation()) != 0)   {
...
void Trainer :: ShowStatus()  {
  ...
  cout << "Being followed by " << numFollowed << " Wild Pokemon" << endl;
}
...
int Trainer :: GetNumFollowed()  {
  return numFollowed;
}

void Trainer :: SetNumFollowed(int num)  {
  numFollowed = num;
}